...
local function pickupCandidates(free, origin, filterFn)
	...
	local function consider(child)
		...
		if filterFn and not filterFn(child, value, weight) then
			return
		end
...
function NewAutoFarm.step()
	...
	local candidates = pickupCandidates(free, root.Position, NewAutoFarm.allowsCrystal)
	if #candidates == 0 then
		requestStream(root.Position)
...
function NewAutoFarm.findFilteredCrystals(origin, radius, free)
	...
	local function consider(inst)
		...
		if not NewAutoFarm.allowsCrystal(inst, value, weight) then
			return
		end
...
function NewAutoFarm.collectFilteredLoot(radius, limit)
	...
	for _, entry in ipairs(found) do
		...
		if entry.weight <= budget and entry.inst.Parent then
			local spot = entry.inst.Position
			requestStream(spot)

			root = getRoot()
			if root and (root.Position - spot).Magnitude > PICK.range then
				local goal = CFrame.new(spot + Vector3.new(0, 5, 0), spot)
...
function NewAutoFarm.discardCrystalTool(tool)
	...
	local function dropped()
		...
		return tool == Workspace or tool:IsDescendantOf(Workspace)
	end
	...
	if DropRequest and pcall(function()
	...
	Library:Notify("Drop failed: Backspace did not drop the tool", 3)
	return false
end
...
function NewAutoFarm.discardStep(force)
	if NewAutoFarm.discardMaxValue <= 0 or NewAutoFarm.discardRangeEnd <= NewAutoFarm.discardMaxValue then
		if force then
			Library:Notify("Set drop value first", 2)
		end
		return 0
	end
...
do
	local function install()
		...
		local LuckFilterBox = Tabs.farming:AddRightGroupbox("Farming Luck Filter", "sparkles")
		...
		LuckFilterBox:AddToggle("NewAutoFarm", {
			Text = "New Auto Pickup",
			Default = false,
			Callback = function(value)
				NewAutoFarm.active = value
				NewAutoFarm.setStatus(value and "New auto pickup enabled" or "New auto pickup disabled")
			end,
		})

		NewAutoFarm.statusLabel = LuckFilterBox:AddLabel("Filter idle", true)

		local AutoDropBox = Tabs.farming:AddRightGroupbox("Auto Drop", "trash")
...
do
	local function install()
		...
		local function step(deltaTime)
			...
			if phase == "loot" then
				...
				if farmMode == "rune" then
					...
					grabbed, found = NewAutoFarm.collectFilteredLoot(RUNE_SWEEP, PICK.burst)
				end
				...
				if pendingFinish then
					...
					anchor = nil
					phase = "scan"
					statusText = farmMode == "rune" and "Next rune boulder" or "Next luck boulder"
				end
		...
		local RuneBox = Tabs.farming:AddLeftGroupbox("Farming Rune", "bot")
		...
		local LuckBox = Tabs.farming:AddRightGroupbox("Farming Luck", "sparkles")
