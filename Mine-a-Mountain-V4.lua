...
local GUI_URL = "https://raw.githubusercontent.com/WinnerShop1/Scripts/refs/heads/main/Mine-a-Mountain-GUI.lua"
...
local function refreshRemotes()
	...
	DropRequest = DropRequest or findRemote("DropRequest") or findRemote("DiscardRequest") or findRemote("DeleteCrystalRequest")
end
...
function NewAutoFarm.setDiscardMaxValue(text)
	local parsed = NewAutoFarm.parseCappedNumber(text, NewAutoFarm.maxValue)
	if parsed then
		NewAutoFarm.discardMaxValue = parsed
		NewAutoFarm.setStatus("Discard <= " .. formatShort(parsed, "$"))
	end
end
...
function NewAutoFarm.discardStep(force)
	if NewAutoFarm.discardMaxValue <= 0 then
		if force then
			Library:Notify("Set discard value first", 2)
		end
		return 0
	end
	...
	NewAutoFarm.eachBackpackCrystalTool(function(tool)
		...
		if value > 0 and value <= NewAutoFarm.discardMaxValue then
	...
	if discarded > 0 then
		NewAutoFarm.setStatus(string.format("Discarded %d item(s) <= %s", discarded, formatShort(NewAutoFarm.discardMaxValue, "$")))
	elseif force then
		NewAutoFarm.setStatus("No backpack item matched discard value")
	end
...
do
	local function install()
		...
		local NewAutoFarmBox = Tabs.farming:AddLeftGroupbox("NewAutofarm", "sparkles")

		NewAutoFarmBox:AddToggle("NewAutoFarm", {
			Text = "New Auto Pickup",
			Default = false,
			Callback = function(value)
				NewAutoFarm.active = value
				NewAutoFarm.setStatus(value and "NewAutofarm enabled" or "NewAutofarm disabled")
			end,
		})

		NewAutoFarmBox:AddDivider()
		NewAutoFarmBox:AddLabel("เก็บเฉพาะแร่ที่ผ่านเงื่อนไข Luck / Weight / Value และระดับแร่ที่เลือก", true)

		NewAutoFarmBox:AddInput("NewAutoFarmMinLuck", {
			Text = "Min Luck",
			Default = "0",
			Placeholder = "1000, 1M",
			Numeric = false,
		...
		NewAutoFarmBox:AddInput("NewAutoFarmMinWeight", {
			Text = "Min Weight",
			Default = "0",
			Placeholder = "1000, 1M",
			Numeric = false,
		...
		NewAutoFarmBox:AddInput("NewAutoFarmMinValue", {
			Text = "Min Value",
			Default = "0",
		...
		NewAutoFarmBox:AddDivider()
		NewAutoFarmBox:AddLabel("Rarity Filter", false)

		for _, rarity in ipairs(NewAutoFarm.rarities) do
			NewAutoFarmBox:AddToggle("NewAutoRarity" .. rarity, {
				Text = rarity,
				Default = true,
				Callback = function(value)
		...
		NewAutoFarmBox:AddInput("NewAutoDiscardMaxValue", {
			Text = "Discard <= Value",
			Default = "0",
			Placeholder = "10B",
			Numeric = false,
		...
		NewAutoFarmBox:AddToggle("NewAutoDiscard", {
			Text = "Auto Discard Backpack",
			Default = false,
			Callback = function(value)
				NewAutoFarm.discardActive = value
				NewAutoFarm.setStatus(value and "Auto discard enabled" or "Auto discard disabled")
			end,
		})

		NewAutoFarmBox:AddButton("Discard Now", function()
			local count = NewAutoFarm.discardStep(true)
			Library:Notify(string.format("Discarded %d item(s)", count), 2)
		end)

		NewAutoFarm.statusLabel = NewAutoFarmBox:AddLabel("Idle", true)
...
do
	local function install()
		...
		local function beginLoot(finish)
			...
			runeLootUntil = now + RUNE_LOOT_TIME
			runeGrabAt = 0
			lootUntil = runeLootUntil + LOOT_TIME
			phase = "loot"
		...
		local function step(deltaTime)
			...
			if phase == "loot" then
				...
				if now < runeLootUntil then
					if now >= runeGrabAt then
						runeGrabAt = now + RUNE_GRAB_STEP
						Mountain.grabNear(RUNE_SWEEP)
					end

					statusText = string.format("Looting runes  %.1fs", runeLootUntil - now)
					return
				end

				local crystalCount, crystalFound = NewAutoFarm.collectFilteredLoot(RUNE_SWEEP, PICK.burst)

				if crystalFound > 0 then
					lootUntil = now + 0.25
					statusText = string.format("Looting filtered crystals  %d found", crystalFound)
					return
				end

				if now < lootUntil then
					statusText = string.format("Looting filtered crystals  %.1fs", lootUntil - now)
					return
				end
				...
				if pendingFinish then
					...
					statusText = "Filtered crystals collected"
				else
					target = nil
					anchor = nil
					phase = "scan"
